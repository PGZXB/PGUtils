// Temp PGTest: a temporary small test framework
...
#define PGTEST_CASE_EX(name, testsInstance) \
    bool test_##name(); \
    bool testAdder_##name = ::pg::test::addTest(testsInstance, #name, test_##name, __FILE__, __LINE__); \
    bool test_##name()

#define PGTEST_CASE(name) PGTEST_CASE_EX(name, pg::test::getDefaultTestsInstance())
